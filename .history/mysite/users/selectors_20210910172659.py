...
def get_strava_last_request_epoc_time(user: get_user_model) -> int:
    ...
    print(user.strava.last_request_epoc_time)
...
def get_activities_from_strava(user: get_user_model, date: datetime) -> dict:
    ...
    if can_request_be_send(strava_obj):
        ...
        return process_request(url, header, 'GET')
    print('Request cannot be send due to lack of information needed or invalid token')
    return None

def can_request_be_send(strava_obj: StravaApi) -> bool:
    """ check whether request can be send with available inforamtions """
    print(is_token_valid(strava_obj))
    print(has_needed_information_for_request(strava_obj))
    print(get_new_strava_access_token(strava_obj))
...
def process_request(strava_obj: StravaApi, url: str, payload: dict, type: str) -> dict:
    ...
    if type == 'GET':
        response = send_get_request_to_strava(url, hedaders=payload)
    else:
