...
def get_activities_from_strava(user: get_user_model, date: datetime) -> dict:
    strava_token_instance = user.strava
...
def can_request_be_send() -> bool:
    """ check whether request can be send with available inforamtions """
    return is_token_valid() or has_needed_information_for_request() and get_new_strava_access_token()
