...
from users.selectors import get_strava_last_request_epoc_time, get_activities_from_strava,   process_and_save_strava_activities, prepare_strava_request_url, create_authorization_header, process_request
import datetime
...
def update_activities(user: get_user_model, date: datetime) -> None:
    ...
    if get_strava_last_request_epoc_time(user=user) > hour:
        ...
        process_and_save_strava_activities(userraw_strava_activities)
...
def process_and_save_strava_activities(raw_strava_activities: list) -> None:
    ...
    if raw_strava_activities and isinstance(raw_strava_activities, list):
        ...
        for activity in raw_strava_activities:
            ...
            raw_activity_details = process_request(url, header, 'GET')
